...
homekit_characteristic_t cha_pm25  = HOMEKIT_CHARACTERISTIC_(PM25_DENSITY, 1);
homekit_characteristic_t cha_aqi  = HOMEKIT_CHARACTERISTIC_(AIR_QUALITY, 1);

homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id=1, .category=homekit_accessory_category_sensor, .services=(homekit_service_t*[]) {
        HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics=(homekit_characteristic_t*[]) {
            HOMEKIT_CHARACTERISTIC(NAME, "环境监测设备"),
            HOMEKIT_CHARACTERISTIC(MANUFACTURER, "Arduino Environment HomeKit"),
        ...
        HOMEKIT_SERVICE(AIR_QUALITY_SENSOR, .primary=true, .characteristics=(homekit_characteristic_t*[]) {
            HOMEKIT_CHARACTERISTIC(NAME, "AQI"),
            &cha_aqi,
            &cha_pm25,
            &cha_name,//optional
            NULL
